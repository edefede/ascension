...
private:
    ...
    Value pop() {
        if (stack.empty()) throw AscensionException("Stack vuoto durante '" + program[ip].op + "'", "VMError");
        Value v = stack.back(); stack.pop_back(); return v;
    }

    void jump(const std::string& lbl) {
        auto it = labels.find(lbl);
        if (it == labels.end()) throw AscensionException("Label non trovata: '" + lbl + "'", "VMError");
        ip = it->second;
    }
...
public:
    ...
    void setAllowExec(bool v) { allowExec = v; }
            ...
            try {
                ...
                if (op == "PUSH") {
                    if (!arg.empty() && arg[0] == '"') {
                        ...
                        try {
                            if (arg.find('.') != std::string::npos) stack.push_back(Value(std::stod(arg)));
                            else stack.push_back(Value(static_cast<int64_t>(std::stoll(arg))));
                        } catch (...) {
                            throw AscensionException("Costante numerica non valida: '" + arg + "'", "VMError");
                        }
                    }
                }
                ...
                else if (op == "STORE") { setVar(arg, pop()); }
                else if (op == "STORE_GLOBAL") { setVar(arg, pop(), true); }
                ...
                // Array 2D
                    ...
                    Value init = pop();
                    int64_t cols = pop().asInt();
                    int64_t rows = pop().asInt();
                ...
                // Built-ins
                    ...
                    if (t.isString()) stack.push_back(Value((int64_t)std::get<std::string>(t.data).size()));
                        ...
                        int64_t c = 0; for (const auto& [k, v] : *t.asDict()) if (k.empty() || k[0] != '_') c++;
                    ...
                    if (c.isDict()) {
                        ...
                        for (const auto& [k, v] : *c.asDict()) if (k.empty() || k[0] != '_') keys.push_back(k);
                ...
                else if (op == "TO_FLOAT") { Value v = pop(); stack.push_back(Value(v.asDouble())); }
                else if (op == "SUBSTR") {
                    ...
                    std::string cmd = pop().asString();
                    if (!allowExec) throw AscensionException("system() disabilitato: avviare con -allow-exec", "SecurityError");
                    stack.push_back(Value(std::system(cmd.c_str())));
                }
                else if (op == "EXEC") {
                    std::string cmd = pop().asString();
                    if (!allowExec) throw AscensionException("exec() disabilitato: avviare con -allow-exec", "SecurityError");
                    std::string out;
                    #ifdef _WIN32
                ...
                // File I/O
                    ...
                    std::string mode = pop().asString();
                    std::string fname = pop().asString();
                    stack.push_back(fileIO.fileOpen(fname, mode));
                }
                else if (op == "FILE_WRITE") {
                    std::string content = pop().asString();
                    int64_t hid = pop().asInt();
                    stack.push_back(fileIO.fileWrite(hid, content));
                }
                else if (op == "FILE_READLINE") { int64_t h = pop().asInt(); stack.push_back(fileIO.fileReadLine(h)); }
                else if (op == "FILE_READALL") { int64_t h = pop().asInt(); stack.push_back(fileIO.fileReadAll(h)); }
                else if (op == "FILE_CLOSE") { int64_t h = pop().asInt(); stack.push_back(fileIO.fileClose(h)); }
                ...
                else if (op == "CURSES_MOVE") { int x = pop().asInt(); int y = pop().asInt(); stack.push_back(curses.move(y, x)); }
                else if (op == "CURSES_WRITE") { std::string s = pop().asString(); stack.push_back(curses.write(s)); }
                else if (op == "CURSES_READ_KEY") { stack.push_back(curses.readKey()); }
                else if (op == "CURSES_MAX_Y") { stack.push_back(curses.getMaxY()); }
                else if (op == "CURSES_MAX_X") { stack.push_back(curses.getMaxX()); }
                
                // Network
                else if (op == "SOCKET_OPEN") { std::string proto = pop().asString(); std::string type = pop().asString(); stack.push_back(network.socketOpen(type, proto)); }
                else if (op == "SOCKET_BIND") { int port = pop().asInt(); std::string ip = pop().asString(); int64_t sid = pop().asInt(); stack.push_back(network.socketBind(sid, ip, port)); }
                else if (op == "SOCKET_LISTEN") { int bl = pop().asInt(); int64_t sid = pop().asInt(); stack.push_back(network.socketListen(sid, bl)); }
                else if (op == "SOCKET_ACCEPT") { int64_t sid = pop().asInt(); stack.push_back(network.socketAccept(sid)); }
                else if (op == "SOCKET_CONNECT") { int port = pop().asInt(); std::string ip = pop().asString(); int64_t sid = pop().asInt(); stack.push_back(network.socketConnect(sid, ip, port)); }
                else if (op == "SOCKET_SEND") { std::string data = pop().asString(); int64_t sid = pop().asInt(); stack.push_back(network.socketSend(sid, data)); }
                else if (op == "SOCKET_RECV") { int mb = pop().asInt(); int64_t sid = pop().asInt(); stack.push_back(network.socketRecv(sid, mb)); }
                else if (op == "SOCKET_CLOSE") { int64_t sid = pop().asInt(); stack.push_back(network.socketClose(sid)); }
                else if (op == "GET_IP") { std::string h = pop().asString(); stack.push_back(network.getIP(h)); }
                else if (op == "HTTP_GET") { std::string url = pop().asString(); stack.push_back(network.httpGet(url)); }
                else if (op == "HTTP_POST") { std::string data = pop().asString(); std::string url = pop().asString(); stack.push_back(network.httpPost(url, data)); }
                
                // Try/Catch
                else if (op == "TRY_START") { tryStack.push_back({arg, localFrames.size(), callStack.size()}); }
                else if (op == "TRY_END") { if (!tryStack.empty()) tryStack.pop_back(); jump(arg); continue; }
                else if (op == "THROW") { std::string msg = stack.empty() ? "Error" : pop().asString(); throw AscensionException(msg); }
                ...
                else if (op == "NOT") { Value v = pop(); stack.push_back(Value(v.toBool() ? 0 : 1)); }
                
                // Math functions
                else if (op == "RANDOM") { std::uniform_real_distribution<double> d(0.0, 1.0); stack.push_back(Value(d(rng))); }
                else if (op == "RANDOM_MAX") { int64_t m = pop().asInt(); if (m <= 0) throw AscensionException("random(max): max deve essere > 0", "MathError"); std::uniform_int_distribution<int64_t> d(0, m - 1); stack.push_back(Value(d(rng))); }
                else if (op == "RANDOM_RANGE") { int64_t mx = pop().asInt(); int64_t mn = pop().asInt(); if (mn >= mx) throw AscensionException("random(min,max): min deve essere < max", "MathError"); std::uniform_int_distribution<int64_t> d(mn, mx - 1); stack.push_back(Value(d(rng))); }
                else if (op == "SQRT") { double v = pop().asDouble(); if (v < 0) throw AscensionException("sqrt negativo", "MathError"); stack.push_back(Value(std::sqrt(v))); }
                else if (op == "POW") { double e = pop().asDouble(); double b = pop().asDouble(); stack.push_back(Value(std::pow(b, e))); }
                else if (op == "EXP") { double v = pop().asDouble(); stack.push_back(Value(std::exp(v))); }
                else if (op == "LOG") { double v = pop().asDouble(); if (v <= 0) throw AscensionException("log non positivo", "MathError"); stack.push_back(Value(std::log(v))); }
                else if (op == "ABS") { stack.push_back(Value(std::abs(pop().asDouble()))); }
                else if (op == "FLOOR") { double v = pop().asDouble(); stack.push_back(Value((int64_t)std::floor(v))); }
                else if (op == "CEIL") { double v = pop().asDouble(); stack.push_back(Value((int64_t)std::ceil(v))); }
                else if (op == "SIN") { double v = pop().asDouble(); stack.push_back(Value(std::sin(v))); }
                else if (op == "COS") { double v = pop().asDouble(); stack.push_back(Value(std::cos(v))); }
                else if (op == "TAN") { double v = pop().asDouble(); stack.push_back(Value(std::tan(v))); }
                else if (op == "ASIN") { double v = pop().asDouble(); if (v < -1 || v > 1) throw AscensionException("asin range", "MathError"); stack.push_back(Value(std::asin(v))); }
                else if (op == "ACOS") { double v = pop().asDouble(); if (v < -1 || v > 1) throw AscensionException("acos range", "MathError"); stack.push_back(Value(std::acos(v))); }
                else if (op == "ATAN") { double v = pop().asDouble(); stack.push_back(Value(std::atan(v))); }
                else if (op == "ATAN2") { double x = pop().asDouble(); double y = pop().asDouble(); stack.push_back(Value(std::atan2(y, x))); }
                
                // Control flow
                else if (op == "JMP") { jump(arg); continue; }
                else if (op == "JZ") { if (!pop().toBool()) { jump(arg); continue; } }
                else if (op == "JNZ") { if (pop().toBool()) { jump(arg); continue; } }
                else if (op == "PRINT") {
                    std::vector<std::string> args;
                    for (int i = 0; i < argCount; i++) args.push_back(pop().asString());
                    std::cout << "OUTPUT > ";
