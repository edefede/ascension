...
private:
    ...
    void parseExpression(const std::string& exprRaw) {
        ...
        if (std::regex_match(expr, std::regex("^curses_rows\\s*\\(\\s*\\)$"))) { ops.push_back(Opcode("CURSES_ROWS")); return; }
        if (std::regex_match(expr, std::regex("^curses_cols\\s*\\(\\s*\\)$"))) { ops.push_back(Opcode("CURSES_COLS")); return; }
